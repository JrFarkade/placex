...
class WeekToggleRequest(BaseModel):
    branch: str
    level: str
    week_num: int
...
@router.get("/branches")
def get_roadmap_branches():
    return LearningService.get_branches_and_levels()

@router.get("/path")
def get_docx_roadmap_path(
    branch: Optional[str] = None,
    level: Optional[str] = None,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
    return LearningService.get_docx_roadmap(
        db=db,
...
def toggle_week_status(
    toggle_in: WeekToggleRequest,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
    return LearningService.update_week_status(
        db=db,
        user_id=current_user.id,
        branch=toggle_in.branch,
        level=toggle_in.level,
        week_num=toggle_in.week_num,
        status=toggle_in.status
    )
