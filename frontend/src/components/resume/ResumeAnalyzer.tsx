...
import { FileText, Upload, Sparkles, CheckCircle2, Loader2, FileX, History, Trash2, HelpCircle, AlertTriangle, Image, Bot, ArrowRight, Lightbulb, Target, AlertCircle } from 'lucide-react';
...
export const ResumeAnalyzer: React.FC<ResumeAnalyzerProps> = ({ token, setActiveFeature }) => {
  ...
  const handleReviewWithHostAgent = async () => {
    ...
    try {
      ...
      setAgentReviewError(err.response?.data?.detail || 'Host Agent review failed. Please try again.');
    } finally {
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {agentReview && agentReview.status === 'success' && (
            <div className="bg-[#FAF8F5] p-6 sm:p-8 rounded-3xl border-2 border-[#0D9488] shadow-md space-y-6 transition-all">
              ...
              {/* 4-Question Framework Section Grid */}
              <div className="space-y-4">
                {/* 1. WHAT */}
                <div className="bg-white p-4 rounded-2xl border border-[#EAE7DF] space-y-1">
                ...
                {/* 2. WHY */}
                <div className="bg-white p-4 rounded-2xl border border-[#EAE7DF] space-y-1">
                  <div className="flex items-center gap-2 text-xs font-extrabold text-[#D97706] uppercase tracking-wider">
                ...
                <div className="bg-white p-4 rounded-2xl border border-[#EAE7DF] space-y-1">
                  <div className="flex items-center gap-2 text-xs font-extrabold text-[#0284C7] uppercase tracking-wider">
                    ...
                    <span>SO WHAT? (CAREER IMPACT)</span>
                  </div>
                  <p className="text-xs text-[#202321] font-medium leading-relaxed">
                    {agentReview.so_what}
                  </p>
                ...
                <div className="bg-white p-4 rounded-2xl border border-[#EAE7DF] space-y-1">
                  <div className="flex items-center gap-2 text-xs font-extrabold text-[#059669] uppercase tracking-wider">
                    ...
                    <span>NOW WHAT? (ACTION STEPS)</span>
                  </div>
                  <p className="text-xs text-[#202321] font-medium leading-relaxed">
                    {agentReview.now_what}
                  </p>
                </div>
              </div>
              ...
              {agentReview.recommendations?.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-xs font-extrabold text-[#949A95] uppercase tracking-wider">Prioritized Recommendations</h4>
                  <div className="space-y-1.5">
                    {agentReview.recommendations.map((rec: string, i: number) => (
                      <div key={i} className="flex items-center gap-2 text-xs font-bold text-[#525753] bg-white px-3 py-2 rounded-xl border border-[#EAE7DF]">
